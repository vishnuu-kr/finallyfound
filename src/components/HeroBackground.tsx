...
export default function HeroBackground({ movies }: { movies: MediaItem[] }) {
  ...
  return (
    <div className="absolute inset-0 z-0 overflow-hidden opacity-[0.15] pointer-events-none flex flex-col gap-3 sm:gap-4 -skew-y-6 scale-125 origin-top-left translate-y-[-10%]">
      ...
      >
        {row1.map((m, i) => (
          <img key={`${m.id}-${i}`} src={m.posterUrl} className="w-24 sm:w-32 md:w-48 h-36 sm:h-48 md:h-72 object-cover rounded-xl" alt="" loading="lazy" />
        ))}
      </motion.div>
